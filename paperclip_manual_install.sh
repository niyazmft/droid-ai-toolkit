...
if command -v curl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    ...
    if [ -n "$LATEST_TAG" ]; then
        ...
        echo -e "\033[0;33m[WARN]\033[0m Could not fetch latest release tag. Falling back to hardcoded v1.15.0."
    fi
fi
TOOLKIT_VERSION=${LATEST_TAG:-"v1.15.0"} # Fallback to last known good version
